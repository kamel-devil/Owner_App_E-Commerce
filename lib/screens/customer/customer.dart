...
class _Customer_ScreenState extends State<Customer_Screen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:FutureBuilder(
          future:  p.getCustomerData(isRefresh: true, search,'103' ),
          builder: (context, snapshot) {
            return SmartRefresher(
              controller: p.refreshCustomer,
              enablePullUp: true,
              onRefresh: () async {
                final result =
                await p.getCustomerData(isRefresh: true, search,'103' );
                if (result) {
                  p.refreshCustomer.refreshCompleted();
                } else {
                  p.refreshCustomer.refreshFailed();
                }
              },
              onLoading: () async {
                final result = await  p.getCustomerData( search,'103' );
                if (result) {
                  p.refreshCustomer.loadComplete();
                } else {
                  p.refreshCustomer.loadFailed();
                }
              },
                ...
                slivers: [
                  SliverList(
                      delegate: SliverChildBuilderDelegate(
                              ...
                              children: [
                                ClipRRect(
                                  ...
                                  child: buildCacheNetworkImage(
                                      width: 100,
                                      height: 100,
                                      url: p.dataCustomer[index]["image"]),
                                ),
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          Expanded(
                                            child: Text(
                                              '${p.dataCustomer[index]["f_name"]}  ${p.dataCustomer[index]["l_name"]}',
                                              maxLines: 1,
                        ...
                        childCount: p.dataCustomer.length,
                      )),
                ],
              ),
            );
      ...
      floatingActionButton: FloatingActionButton(
        ...
        Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => const addCustomer_Screen()));
        },
        child: Container(
            width: 60,
            height: 60,
